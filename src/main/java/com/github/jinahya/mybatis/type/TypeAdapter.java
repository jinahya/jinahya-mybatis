...
public class TypeAdapter<T extends TypeConverter<X, Y>, X, Y>
        extends AbstractTypeAdapter<T, X> {

    // -------------------------------------------------------------------------
    public TypeAdapter(final Class<T> converterClass) {
    ...
    public X getNullableResult(final ResultSet rs, final String columnName)
            throws SQLException {
        return converter().toAttributeType((Y) rs.getObject(columnName));
    }
    ...
    public X getNullableResult(final ResultSet rs, final int columnIndex)
            throws SQLException {
        return converter().toAttributeType((Y) rs.getObject(columnIndex));
    }
    ...
    public X getNullableResult(final CallableStatement cs,
            ...
            throws SQLException {
        return converter().toAttributeType((Y) cs.getObject(columnIndex));
    }
}
